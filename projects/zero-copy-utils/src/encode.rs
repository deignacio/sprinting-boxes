...
unsafe fn extract_sample_buffer_data(sbuf: &CMSampleBuffer) -> Result<(Vec<u8>, i64, i64, bool)> {
    ...
    // VT HEVC encoder returns kCMTimeInvalid for DTS when B-frames are disabled.
    // kCMTimeInvalid has the Valid flag clear (value is 0, not i64::MIN).
    // Fall back to PTS so the muxer computes the correct track duration.
    let dts_val = if dts.flags.contains(CMTimeFlags::Valid) {
        dts.value
    } else {
        pts_val
    };
...
unsafe fn extract_hevc_extradata(sbuf: &CMSampleBuffer) -> Result<Vec<u8>> {
    ...
    // Look up "hvcC" key (HEVCDecoderConfigurationRecord).
