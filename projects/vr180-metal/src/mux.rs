...
impl Muxer {
    pub fn open(
        ...
        // --- Video stream (HEVC) ---
        // Use the input stream's timebase so that PTS values passed from decoded frames
        // are interpreted correctly (e.g. 1/30000 with PTS steps of 1001 for ~29.97 fps).
        ...
        unsafe {
            let mut par = video_stream.parameters();
            (*par.as_mut_ptr()).codec_type = ffmpeg_next::ffi::AVMediaType::AVMEDIA_TYPE_VIDEO;
            (*par.as_mut_ptr()).codec_id = ffmpeg_next::ffi::AVCodecID::AV_CODEC_ID_HEVC;
            (*par.as_mut_ptr()).width = out_width as i32;
            (*par.as_mut_ptr()).height = out_height as i32;
            (*par.as_mut_ptr()).format = ffmpeg_next::ffi::AVPixelFormat::AV_PIX_FMT_YUV420P as i32;
        }
        let video_stream_idx = video_stream.index();

        // --- Audio stream: copy from input ---
        let input_ctx = format::input(input_path).context("open input for audio")?;
        ...
        // NOTE: write_header() is called later via write_header_with_extradata()
        // once we have the encoder's HEVCDecoderConfigurationRecord extradata.
