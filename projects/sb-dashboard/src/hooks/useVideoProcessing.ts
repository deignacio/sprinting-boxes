...
export const useVideoProcessing = (id: string | undefined, run: RunDetail | null, fetchRun: () => void) => {
    ...
    const connectToSSE = useCallback(() => {
        ...
        eventSource.onmessage = (event) => {
            try {
                ...
                const data = JSON.parse(event.data) as ProcessingProgress | { error: string };
    ...
    const handleComputeCrops = async () => {
        ...
        try {
            ...
            setProcessingError(e instanceof Error ? e.message : "Failed to compute crops");
        }
    };
