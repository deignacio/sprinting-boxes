...
import { computeROI, normalizeBoundaries } from "../../utils/boundaryUtils";
import type {
  Boundaries,
  BoundaryKey,
    ...
    try {
      ...
      if (list.length === 0) {
        ...
        const extractRes = await fetch(
          `/api/runs/${runId}/calibration/extract`,
          { method: "POST" },
        );
        if (!extractRes.ok)
          throw new Error("Failed to extract calibration frames");
